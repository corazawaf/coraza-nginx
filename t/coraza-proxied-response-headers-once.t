...
# see it twice.
#
# The oracle counts each dedicated header independently and denies unless each
# was seen exactly once. A passing 200 proves single submission; a duplicate or
# omitted submission would flip this to 403.
#
    ...
    server {
        ...
        location /proxied {
            ...
            coraza_rules '
                ...
                SecAction "id:400,phase:1,pass,nolog,t:none,setvar:tx.server_hits=0,setvar:tx.date_hits=0,setvar:tx.last_modified_hits=0"
                SecRule RESPONSE_HEADERS:Server "@rx ." "id:401,phase:3,pass,log,setvar:tx.server_hits=+1,t:none"
                SecRule RESPONSE_HEADERS:Date "@rx ." "id:402,phase:3,pass,log,setvar:tx.date_hits=+1,t:none"
                SecRule RESPONSE_HEADERS:Last-Modified "@rx ." "id:403,phase:3,pass,log,setvar:tx.last_modified_hits=+1,t:none"
                SecRule TX:SERVER_HITS "!@eq 1" "id:494,phase:3,deny,log,status:403,t:none"
                SecRule TX:DATE_HITS "!@eq 1" "id:495,phase:3,deny,log,status:403,t:none"
                SecRule TX:LAST_MODIFIED_HITS "!@eq 1" "id:496,phase:3,deny,log,status:403,t:none"
            ';
        ...
        location /control {
            ...
            coraza_rules '
                SecRuleEngine On
                SecResponseBodyAccess Off
                SecRule RESPONSE_HEADERS:X-Dup "@rx ." "id:411,phase:3,pass,log,setvar:tx.dup_hits=+1,t:none"
                SecRule TX:DUP_HITS "@gt 1" "id:498,phase:3,deny,log,status:403,t:none"
            ';
            proxy_pass http://127.0.0.1:%%PORT_8081%%;
        }
...
like(http_get('/proxied'), qr/^HTTP\S+ 200/,
    'proxied Server/Date/Last-Modified each counted exactly once');

my $log = $t->read_file('error.log');
unlike($log, qr/id "49[4-6]"/,
    'no dedicated-header cardinality guard fired');
